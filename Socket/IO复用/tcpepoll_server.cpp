...
int main(int argc, char *argv[])
{
    ...
    // 创建一个描述符
    epollfd = epoll_create(1);//epoll_create参数只需要大于1即可。

    // 添加监听描述符事件
    struct epoll_event ev;
    ev.data.fd = listensock;
    ev.events = EPOLLIN;
    //将listensock以及事件加到前面epoll_create创建的epollf描述符中，
    ...
    while (1)
    {
        ...
        // 等待监视的socket有事件发生。
        //将描述符epollfd中有事件发生的按最大值MAXEVENTS给到events里，如果发生事件为200，MAXEVENTS=100，那么另外的100个事件会在第一个100结束后立即返回，不会发生丢失。
        int infds = epoll_wait(epollfd, events, MAXEVENTS, -1);//返回值就是epollfd中发生事件的个数
        ...
        // 遍历有事件发生的结构数组。
        //这个地方区别与select和poll，这里遍历的时所有发生事件的描述符；而select和poll遍历的是所有的，而只响应发生事件的描述符
