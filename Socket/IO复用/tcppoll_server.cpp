...
//poll处理数组的socket时不需要创建临时副本，因为poll响应事件时不会修改原始数组的前两个参数，只会修改第三个参数returned events（）.而poll每次响应要用的都是前两个参数
// ulimit -n
#define MAXNFDS 1024
...
int main(int argc, char *argv[])
{
    ...
    //打开文件个数不光受限于MAXNFDS的大小还受限于系统可打开文件的限制。ulimit -n查看
    //pollfd结构体中fd的值若为负数则会被忽略，这也是下面将fd设为1的原因
    ...
    // 把listensock添加到数组中。
    //这个地方要理解数组fds的结构才好理解为什么用这种方法赋值
    fds[listensock].fd = listensock;
    //下面的事件可赋的值有多种，我们一般只用POLLIN
    fds[listensock].events = POLLIN; // 有数据可读事件，包括新客户端的连接、客户端socket有数据可读和客户端socket断开三种情况。
    maxfd = listensock;

    while (1)
    {
        //poll的第一个参数是文件描述符数组集，也就是socket。
        //第二个参数是数组集中有效文件描述符个数
        //第三个参数是超时机制设置
